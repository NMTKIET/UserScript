...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // === Logic cho Editor View (đã có từ trước, thêm một số chỉnh sửa nhỏ) ===
...
(function() {
    ...
    userScripts.push({
      ...
      enabled: true,
      version: "1.0",
      size: `${new TextEncoder().encode(templateCode).length / 1024}`.slice(0, 3) + " KB", // Approximate size
  ...
  saveScriptBtn.onclick = () => {
    ...
    userScripts[currentScriptIndex].size = `${new TextEncoder().encode(updatedCode).length / 1024}`.slice(0, 3) + " KB";
  ...
  function renderUserscriptsTable() {
    chrome.storage.local.get(['userScripts'], (result) => {
      ...
      userScripts.forEach((script, index) => {
        ...
        // Size column
        row.insertCell().textContent = script.size || `${new TextEncoder().encode(script.code).length / 1024}`.slice(0, 3) + " KB";
  ...
  // === Khởi tạo ban đầu ===
  showTab('list-view'); // Hiển thị tab danh sách script khi popup mở
});
